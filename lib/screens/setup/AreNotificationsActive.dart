...
class AreNotificationsActive {
  ...
  Future<void> _loadSettings() async {
    _selectedTimeZone = await secureStorage.readSecureData('timeZone') ?? 'Europe/Berlin';
  ...
  Future<bool> areNotificationsActive() async {
    ...
    final currentDayOfWeek = now.weekday - 1; // DateTime's weekday is 1-based (Monday is 1, Sunday is 7)
    ...
    if (_selectedDays[currentDayOfWeek]) {
      ...
      final isNowDuringWorkingHours =
          (currentTime.hour > _workingHoursStart.hour || (currentTime.hour == _workingHoursStart.hour && currentTime.minute >= _workingHoursStart.minute)) && (currentTime.hour < _workingHoursEnd.hour || (currentTime.hour == _workingHoursEnd.hour && currentTime.minute <= _workingHoursEnd.minute));
      ...
      return isNowDuringWorkingHours ? _notifyDuringWorkingHours : _notifyDuringOffHours;
    }
