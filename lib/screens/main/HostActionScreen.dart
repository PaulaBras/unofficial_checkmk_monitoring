...
class _HostActionScreenState extends State<HostActionScreen> {
  ...
  void acknowledgeHost(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              AcknowledgeHostForm(service: widget.host)),
    );
  }
  ...
  Future<void> _getHost() async {
    ...
    var data = await api.Request(
        'domain-types/host/collections/all?query=%7B%22op%22%3A%20%22%3D%22%2C%20%22left%22%3A%20%22name%22%2C%20%22right%22%3A%20%22Bonn1%22%7D&columns=name&columns=address&columns=last_check&columns=last_time_up&columns=state&columns=total_services&columns=acknowledged');
  ...
  Future<List<dynamic>> _getComments() async {
    var api = ApiRequest();
    var data = await api.Request(
        'domain-types/comment/collections/all?host_name=${widget.host['extensions']['host_name']}');
    return data['value'];
  }
