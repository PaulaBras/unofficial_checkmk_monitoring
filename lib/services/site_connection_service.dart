...
class SiteConnectionService {
  ...
  Future<List<SiteConnection>> getAllConnections() async {
    try {
      final String? connectionsJson =
          await _secureStorage.readSecureData(_connectionsKey);
    ...
    try {
      ...
      try {
        ...
        if (server != null &&
          ...
          final ignoreCertificate =
              (await _secureStorage.readSecureData('ignoreCertificate'))
                      ?.toLowerCase() ==
                  'true';
          final enableNotifications =
              (await _secureStorage.readSecureData('enableNotifications'))
                      ?.toLowerCase() ==
                  'true';
          ...
          for (var state in ['green', 'warning', 'critical', 'unknown']) {
            ...
            serviceStateNotifications[state] =
  ...
  Future<bool> handleLogout() async {
    try {
      ...
      final currentIndex =
          connections.indexWhere((c) => c.id == activeConnectionId);
      if (currentIndex == -1) {
      ...
      if (updatedConnections.isNotEmpty) {
        ...
        final nextIndex =
            currentIndex >= updatedConnections.length ? 0 : currentIndex;
